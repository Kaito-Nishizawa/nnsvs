...
class ResF0NonAttentiveDecoder(BaseModel):
    ...
    def forward(self, encoder_outs, in_lens, decoder_targets=None):
        ...
        """
        is_inference = decoder_targets is None  # True
        if not is_inference:
        ...
        if not is_inference and self.prenet is not None:  # False
        ...
        for t in range(encoder_outs.shape[1]):
            # Pre-Net
            if self.prenet is not None:  # False
                if is_inference:
        ...
        # outsは変わる
        # print(outs[0])  # tensor([[[-1.9358, -1.9633, -1.9418, -1.8981]]])
...
class BiLSTMResF0NonAttentiveDecoder(BaseModel):
    ...
    def forward(self, x, lengths=None, y=None):
        ...
        lf0_score = x[:, :, self.in_lf0_idx].unsqueeze(-1)  # ここは一緒
        ...
        out, _ = self.lstm(sequence)  # ここは一緒
        out, _ = pad_packed_sequence(out, batch_first=True)  # ここは一緒
