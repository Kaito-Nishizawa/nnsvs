...
"""
    ...
    def predict_timing(self, labels, fluc=[False, False]):
        ...
        Returns:
            nnmnkwii.io.hts.HTSLabelFile: duration modified HTS labels.
        """
        lag = self.predict_timelag(labels, fluc=fluc[0])
        durations = self.predict_duration(labels, fluc=fluc[1])
        duration_modified_full_labels = self.postprocess_duration(
            labels, durations, lag
        )
        return duration_modified_full_labels

    def predict_acoustic(self, duration_modified_labels, f0_shift_in_cent=0, fluc=[False, False, False]):
...
WORLD is only supported for waveform generation"""
    ...
    def svs(
        self,
        labels,
        fluc=[False, False, False, False, False],
        vocoder_type="world",
        post_filter_type="gv",
        trajectory_smoothing=True,
        trajectory_smoothing_cutoff=50,
        ...
        duration_modified_labels = self.predict_timing(labels, fluc=fluc[0:2])
